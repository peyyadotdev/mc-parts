...
function shouldEvaluateAttribute(
	...
	if (definition.sourceFields && !definition.sourceFields.includes(field)) {
		return false;
	}
...
interface PersistOptions {
	...
	transaction?: Parameters<NodePgDatabase["transaction"]>[0];
}
...
export async function persistExtractionResult({
	...
	const runner = transaction
		? transaction
		: (cb: Parameters<NodePgDatabase["transaction"]>[0]) => db.transaction(cb);

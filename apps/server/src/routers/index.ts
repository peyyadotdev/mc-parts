...
export const appRouter = router({
	...
	listAttributeDefinitions: publicProcedure.query(async () => {
		...
		for (const entry of categories) {
			...
			const list =
				categoriesByDefinition.get(entry.attributeDefinitionId) ?? [];
			list.push(label);
		...
		.query(async ({ input }) => {
			...
			const variants = await db
				.select({
					...
					lastExtractedAt: sql<
						string | null
					>`max(case when ${variantAttribute.source} = 'extracted' then ${variantAttribute.extractedAt} end)`,
				})
				.from(productVariant)
	...
	updateVariantAttributes: publicProcedure
		...
		.mutation(async ({ input }) => {
			...
			await persistManualAttributes({
				...
				existingLegacyAttributes: (variant.attributes ?? {}) as Record<
					string,
					unknown
				>,
				clearSlugs: clear,
	...
	extractVariantAttributes: publicProcedure
		...
		.mutation(async ({ input }) => {
			...
			const categories = await db
				...
				.innerJoin(
					categoryTable,
					eq(productCategory.categoryId, categoryTable.id),
				)
			...
			const outcome = extractAttributes(
				{
					...
					legacyAttributes: variant.attributes as Record<
						string,
						unknown
					> | null,
				},
	...
	bulkExtractBrands: publicProcedure
		.input(
			z.object({
		...
		.mutation(async ({ input }) => {
			const { bulkExtractBrands } = await import(
				"../scripts/bulk-brand-extraction"
			);

			const results = await bulkExtractBrands(input.limit, input.dryRun);

			return {
				success: true,
				processed: results.length,
				message: `Processed ${results.length} products for brand extraction`,
	...
	bulkAssignCategories: publicProcedure
		...
		.mutation(async ({ input }) => {
			const { bulkAssignCategories } = await import(
				"../scripts/bulk-category-assignment"
			);

			const results = await bulkAssignCategories(input.limit, input.dryRun);

			return {
				success: true,
				processed: results.length,
				message: `Processed ${results.length} products for category assignment`,
				results: results.slice(0, 10),
			};
		}),

	bulkGenerateDescriptions: publicProcedure
		.input(
			z.object({
		...
		.mutation(async ({ input }) => {
			const { bulkGenerateDescriptions } = await import(
				"../scripts/bulk-description-generation"
			);

			const results = await bulkGenerateDescriptions(input.limit, input.dryRun);

			return {
				success: true,
				processed: results.length,
				message: `Generated descriptions for ${results.length} products`,
				results: results.slice(0, 5),
			};
		}),

	bulkCleanupNames: publicProcedure
		.input(
			z.object({
				limit: z.number().min(1).max(100000).default(1000),
				dryRun: z.boolean().default(false),
		...
		.mutation(async ({ input }) => {
			const { bulkCleanProductNames } = await import(
				"../scripts/bulk-name-cleanup"
			);

			const results = await bulkCleanProductNames(input.limit, input.dryRun);

			return {
				success: true,
				processed: results.length,
				message: `Cleaned up ${results.length} product names`,
				results: results.slice(0, 10),
			};
		}),

	analyzeProductData: publicProcedure.query(async () => {
		const { analyzeProductData } = await import(
			"../scripts/analyze-product-data"
		);
	...
	runFullEnrichment: publicProcedure
		.input(
			z.object({
		...
		.mutation(async ({ input }) => {
			// This would orchestrate the full enrichment workflow
			// For now, we'll return a placeholder
			return {
				success: true,
				message: "Full enrichment workflow would be executed here",

...
function computeStatistics(results: ExtractionTestResult[]): ExtractionStats {
	...
	const withAttributes = results.filter(
		(result) => result.totalAttributes > 0,
	).length;
	...
	const aggregate = results.reduce(
		(state, result) => {
			...
			for (const category of result.categories) {
				const performance = state.categoryPerformance[category] ?? {
					products: 0,
					attributes: 0,
					values: 0,
					confidence: 0,
...
function saveReports(results: ExtractionTestResult[], stats: ExtractionStats) {
	...
	return {
		markdownReport,
		htmlReportPath: `reports/ATTRIBUTE-EXTRACTION-REPORT-${timestamp}.html`,
	};
}
	...
	.map(
		(attribute) =>
			`- ${attribute.slug}: ${attribute.values
				.map(
					(value) =>
						`${value.normalizedValue} (${(value.confidence * 100).toFixed(0)}%)`,
				)
				.join(", ")}`,
	)
	.join("\n")}
...
- **Products with attributes**: ${stats.withAttributes} (${(
		(stats.withAttributes / stats.totalProducts) * 100
	).toFixed(1)}%)
...
async function main() {
	...
	console.log(
		`   Avg attributes/product: ${stats.averageAttributes.toFixed(2)}`,
	);
	console.log(
		`   Avg confidence: ${(stats.averageConfidence * 100).toFixed(1)}%`,
	);
	console.log(
		`   Reports written to data/nyehandel/sample-2025-11-11/${htmlReportPath.replace(/^reports\//, "reports/")}`,
	);
}

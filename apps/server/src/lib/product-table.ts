...
export type ProductTableResponse = {
	...
	meta: {
		...
		chartData: Array<{ timestamp: number; success: number; warning: number; error: number }>;
...
export async function fetchProductTable(
	...
	const rows = await db
		.select({
			...
			categories: sql<string[] | null>`array_remove(array_agg(distinct ${category.name}), NULL)`,
	...
	const mapped = trimmed.map((row) => {
		const price = typeof row.priceCents === "number" ? row.priceCents / 100 : null;
...
async function buildChartData(where?: SQL) {
	...
	const map = new Map<number, { success: number; warning: number; error: number }>();
...
async function buildCategoryFacet(where?: SQL) {
	...
	const categoryWhere = categoryClauses.length ? and(...categoryClauses) : undefined;
	const rows = await db
		.select({
			value: category.name,
			total: sql<number>`count(distinct ${product.id})::int`,
...
function deriveEnrichmentState(raw: string) {
	try {
		...
		const hasManual = attributeEntries.some((attr) => attr?.source === "manual");
...
function mapStatusToLevel(status: string | null | undefined): "success" | "warning" | "error" {

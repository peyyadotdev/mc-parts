<!-- 758dca4c-97fe-400e-a77e-bd3a1a60cab7 440b6de6-39ff-4cf4-a758-a71c8e1c5113 -->
...
- **markdown-storage**: Store markdown + front-matter in product_enrichment (JSONB) while prepping Payload; parse with Gray Matter in server.
- **authoring-ui**: Add markdown editor/preview component with front-matter form + Zod validation for authors in admin UI.
- **render-pipeline**: Deliver markdown→HTML renderer that injects dynamic placeholders (price, stock, availability) when serving product detail previews.

## Phase 4 – Payload & Workflow Automation

- **payload-bootstrap**: Scaffold Payload CMS service (new workspace) backed by Supabase; align collection schemas with Drizzle types.
- **sync-bridge**: Build sync adapters (webhooks or worker polling) to keep Payload edits and PIM tables in lockstep.
- **bullmq-queue**: Introduce worker app with BullMQ for imports, enrichment sync, fitment jobs; add queue status panel in admin UI.
- **trigger-dev (optional)**: Leverage Trigger.dev for scheduled jobs/alerts (nightly syncs, QA reports) once baseline queue is stable.

## Phase 5 – Search, Facets & Reporting

- **pg-search**: Provide initial search endpoint using Postgres trigram/ILIKE filters (brand, template status) and wire into product grid.
- **meili-eval**: Prototype MeiliSearch indexing for richer relevance; allow switching backends in UI for benchmarking.
- **facets-dashboard**: Build reporting dashboard (TanStack Table/Charts) tracking enrichment coverage, attribute completeness, brand normalization progress.

## Phase 6 – Observability, QA & Launch Prep

- **axiom-observability**: Pipe Pino logs from server/worker to Axiom; configure dashboards + alerts for sync failures and validation errors.
- **test-suite**: Expand Vitest unit/integration coverage (schema validation, markdown renderer, template enforcement) and add Playwright smoke tests for critical admin flows.
- **docs-fumadocs**: Publish how-tos in apps/fumadocs (content workflow, sync operations, troubleshooting).
- **deployment-runbook**: Finalize Vercel/Payload deploy process, GitHub Actions pipeline (lint/tests/migrations/content validation), and pilot release checklist.
